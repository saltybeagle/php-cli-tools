...
namespace PEAR2\Console\Tools;
	 ...
	 * @param mixed   ...   Either scalar arguments or a single array argument.
	 * @return void
	 * @see \PEAR2\Console\Tools\render()
	 */
	function out($msg) {
	    $args = func_get_args();
	    fwrite(STDOUT, call_user_func_array('\\PEAR2\\Console\\Tools\\render', $args));
	}
	
	/**
	 * Pads `$msg` to the width of the shell before passing to `PEAR2\Console\Tools\out`.
	 *
	...
	function out_padded($msg) {
	    $args = func_get_args();
	    $msg = call_user_func_array('\\PEAR2\\Console\\Tools\\render', $args);
	    \PEAR2\Console\Tools\out(str_pad($msg, \PEAR2\Console\Tools\Shell::columns()));
	}
	
	/**
	 * Prints a message to `STDOUT` with a newline appended. See `\PEAR2\Console\Tools\out` for
	 * more documentation.
	 *
	 * @see PEAR2\Console\Tools\out()
	 */
	function line($msg = '') {
	    ...
	    $args[0] .= "\n";
	    call_user_func_array('\\PEAR2\\Console\\Tools\\out', $args);
	}
	...
	function err($msg = '') {
	    ...
	    $args = array_merge(func_get_args(), array(''));
	    $args[0] .= "\n";
	    fwrite(STDERR, call_user_func_array('\\PEAR2\\Console\\Tools\\render', $args));
	}
	...
	function prompt($question, $default = false, $marker = ': ') {
	    ...
	    while (true) {
	        \PEAR2\Console\Tools\out($question . $marker);
	        $line = input();
	 ...
	 * @see PEAR2\Console\Tools\prompt()
	 */
	 ...
	 * @see PEAR2\Console\Tools\line()
	 * @see PEAR2\Console\Tools\input()
	 * @see PEAR2\Console\Tools\err()
	 */
